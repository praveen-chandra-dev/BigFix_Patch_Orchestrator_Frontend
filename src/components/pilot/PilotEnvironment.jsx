...
export default function PilotEnvironment({ mode = "pilot", lastActions = {} }) {
  ...
  async function loadOptions() {
    ...
    try {
      ...
      const groupObjects = (gRes.groups || []).sort((a, b) =>
        a.name.localeCompare(b.name),
      );
      const baselineObjects = (bRes.baselines || []).sort((a, b) =>
        a.name.localeCompare(b.name),
      );
  ...
  function restoreDeployments(config = {}) {
    setEnv((f) => {
      const st =
        config.successThreshold != null
          ? Number(config.successThreshold)
          : (f.successThreshold ?? 90);

      const hf =
        config.allowableCriticalHF != null
          ? Number(config.allowableCriticalHF)
      ...
      const isCurrentEmpty =
        ...
        (currentDeps.length === 1 &&
          !currentDeps[0].baseline &&
          !currentDeps[0].group);

      if (isCurrentEmpty) {
        const prevKey = inProduction
          ? "pilotDeployments"
          : "sandboxDeployments";
        ...
        const prevActions = inProduction
          ? lastActions?.PILOT?.actions || lastActions?.SANDBOX?.actions
          : lastActions?.SANDBOX?.actions;

        if (validPrev.length > 0) {
          currentDeps = validPrev.map((d) => ({ ...d }));
        } else if (prevActions?.length) {
          currentDeps = prevActions.map((a) => ({
            baseline: a.baseline,
            group: a.group,
          }));
        } else {
      ...
      return {
        ...
        snapshotVM: config.snapshotVM ?? f.snapshotVM,
        cloneVM: config.cloneVM ?? f.cloneVM,
        enablePilot: config.enablePilot ?? f.enablePilot,
        enableSandbox: config.enableSandbox ?? f.enableSandbox,
  ...
  const isFirstStage = inProduction
    ? !env.enableSandbox && !env.enablePilot
    : !env.enableSandbox;
  const inputsLocked = isFirstStage ? false : !env[`${mode}Unlocked`];
  ...
  const handleAddDeployment = () => {
    ...
    setEnv((p) => ({
      ...p,
      [depKey]: [...(p[depKey] || []), { baseline: "", group: "" }],
    }));
  };
  ...
  return (
    <section className="card reveal mb-0" id="card-env" data-reveal>
      ...
      <div
        className={`env-inputs-row ${loading ? "opacity-60" : ""}`}
        style={{
          display: "flex",
      ...
      >
        ...
        >
          ...
          >
            Deployments (Baseline ➔ {inProduction ? "Production" : "Pilot"}{" "}
            Group)
          </span>
            ...
            >
              <div style={{ flex: 1 }}>
                <FancySelect
                  options={baselines.map((b) => ({
                    value: b.name,
                    label: `${b.name} [${b.component_count ?? 0}]`,
                  }))}
                  value={dep.baseline}
              ...
              <div style={{ flex: 1 }}>
                <FancySelect
                  ...
                  disabled={disabled || !groups.length || inputsLocked}
                  placeholder={
                    !groups.length ? "— loading… —" : "— select group —"
                  }
                  searchable={true}
                />
              </div>
