...
//         // 🚀 NEW: Array Inheritance Logic
//         let currentDeps = f[depKey] || [];
        
//         // If the current stage (Pilot/Prod) is empty, inherit from the previous stage
...
//         //         // Deep copy the previous stage's deployments
//         //         currentDeps = validPrev.map(d => ({ ...d }));
//         //     } else {
//         //         // Fallback to database config if page was hard refreshed
//         //         if (inProduction && cConfig.lastProdBaseline) {
//         //             currentDeps = [{ baseline: cConfig.lastProdBaseline, group: cConfig.lastProdGroup || "" }];
//         //         } else if (!inProduction && cConfig.lastPilotBaseline) {
//         //             currentDeps = [{ baseline: cConfig.lastPilotBaseline, group: cConfig.lastPilotGroup || "" }];
//         //         } else if (cConfig.lastSandboxBaseline) {
//         //             currentDeps = [{ baseline: cConfig.lastSandboxBaseline, group: cConfig.lastSandboxGroup || "" }];
//         //         } else {
...
//         if (isCurrentEmpty) {
//             // First try active memory
//             const prevKey = inProduction ? "pilotDeployments" : "sandboxDeployments";
//             const prevDeps = f[prevKey] || [];
//             const validPrev = prevDeps.filter(d => d.baseline || d.group);
            
//             // Second try database-backed Orchestrator history (Survives Refresh)
...
//                 // Instantly reconstruct the UI from DB History
...
//         // Safety net to ensure at least one empty row renders
//         if (currentDeps.length === 0) currentDeps = [{ baseline: "", group: "" }];

//         return {
//           ...f,
//           [depKey]: currentDeps, // 🚀 Apply the inherited array
//           successThreshold: st,
//           allowableCriticalHF: hf,
//           snapshotVM: cConfig.snapshotVM ?? f.snapshotVM,
//           cloneVM: cConfig.cloneVM ?? f.cloneVM,
...
//       {/* 🚀 Changed to a standard column div to completely avoid CSS Grid conflicts */}
