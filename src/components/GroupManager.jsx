...
export default function GroupManager({ onClose }) {
  ...
  const handleExport = (fmt) => { 
    ...
    if (fmt === 'CSV') {
        ...
        const rows = sortedComputers.map(p => cols.filter(c => c.show).map(c => {
           let val = p[c.id];
        ...
        a.href = url; a.download = "computers_export.csv"; a.click();
    }
  };
                ...
                <div className="dropdown" ref={expRef}>
                    ...
                    {showExpDrop && (
                        <div className="dropdown-menu show" style={{ width: "280px", padding: "16px", right: 0 }}>
                            ...
                            <button className="item" onClick={() => handleExport('CSV')}>Current Page</button>
                            <button className="item" onClick={() => handleExport('CSV')}>Filtered Data</button>
                            <button className="item" onClick={() => handleExport('CSV')}>All Data</button>
                        </div>
                    )}
                </div>
             </div>
