...
export default function RiskModule({ onClose, activeTab = "patches", activeSubTab = "overview", setRiskTab, setRiskSubTab, onSetPending }) {
  ...
  // Clear filters dynamically whenever the user navigates between entirely different tabs/sub-tabs
  useEffect(() => {
    setFilters([]);
  ...
  const propertyOptions = useMemo(() => {
    if (activeTab === 'patches') return [ { value: "patch_id", label: "Patch ID" }, { value: "patch_name", label: "Name" }, { value: "severity", label: "Severity" }, { value: "cve_id", label: "CVE ID" }, { value: "final_score", label: "Score" } ];
    if (activeTab === 'baseline') return [ { value: "baseline_name", label: "Baseline Name" }, { value: "patch_id", label: "Patch ID" }, { value: "cve_id", label: "CVE ID" } ];
    if (activeTab === 'dashboard') {
        if (activeSubTab === 'cve') return [{ value: "cve_id", label: "CVE ID" }, { value: "patch_id", label: "Patch ID" }, { value: "device_name", label: "Device Name" }, { value: "kev", label: "KEV" }];
        if (activeSubTab === 'patch') return [{ value: "patch_id", label: "Patch ID" }, { value: "patch_name", label: "Name" }, { value: "severity", label: "Severity" }, { value: "cve_id", label: "CVE ID" }, { value: "device", label: "Device" }, { value: "final_score", label: "Score" }];
        if (activeSubTab === 'computer') return [{ value: "device_name", label: "Device Name" }, { value: "patch_id", label: "Patch ID" }, { value: "cve_id", label: "CVE ID" }];
        if (activeSubTab === 'baseline') return [{ value: "baseline_name", label: "Baseline Name" }, { value: "patch_id", label: "Patch ID" }, { value: "cve_id", label: "CVE ID" }];
    }
    return [];
  }, [activeTab, activeSubTab]);
  ...
  // Hide the filter button specifically when on the Dashboard Overview tab
  const showFilter = !(activeTab === 'dashboard' && activeSubTab === 'overview');
  ...
  return (
    <div className="card reveal" style={{ minHeight: '100%', display: 'flex', flexDirection: 'column', padding: 0, overflow: 'visible', boxShadow: 'none', border: 'none', background: 'transparent' }}>
      ...
      <div style={{ position: 'sticky', top: '-24px', background: 'var(--panel)', zIndex: 20, padding: '24px 32px 16px', borderBottom: '1px solid var(--border)', margin: '-24px -32px 24px -32px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: "22px", fontWeight: 600, color: "var(--text)" }}>Risk Prioritization</h2>
        ...
        <div className="risk-content" style={{ flex: 1 }}>
          ...
          {activeTab === "dashboard" && (
            <DashboardTab 
              ...
              onDataLoaded={() => { if(!lastUpdated) setLastUpdated(new Date().toLocaleString()); }}
            />
          )}
        </div>
      </div>
