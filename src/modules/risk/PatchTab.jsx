  ...
  const loadPatchCves = async (patch) => {
    ...
    try {
      const res = await api.post("/cves/by-patches", {
        patches: [{ patch_id: patch.patch_id, site_name: patch.site_name }],
      });
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
            ...
            <tr>
              ...
              {cols.find((c) => c.id === "applicable_count")?.show && (
                <th
                  ...
                  className="cursor-pointer"
                >
                  Applicable Computers{getSortIcon("applicable_count")}
                </th>
              )}
              {cols.find((c) => c.id === "cve_count")?.show && (
                <th
                  ...
                  className="cursor-pointer"
                >
                  Associated CVE IDs{getSortIcon("cve_count")}
                </th>
              )}
      ...
      >
        ...
        <span
          ...
          style={{ fontSize: "13px", color: "var(--muted)" }}
        >
          {filteredPatches.length > 0 ? (currentPage - 1) * rowsPerPage + 1 : 0}
          -{Math.min(currentPage * rowsPerPage, filteredPatches.length)} of{" "}
